...
describe('StarService 클래스', () => {
    ...
    describe('addPointToAuthor 메소드에서', () => {
        it('스스에게 별을 보내면 점수가 추가되지 않는다', async () => {
            await sut.addPointToAuthor(testSpaceURL, testClientId, testUserId, testMessageId, testAuthor, getTestAxiosOption())
        ...
        it('유저가 누른 리액션이 아니면 점수가 추가되지 않는다', async () => {
            await sut.addPointToAuthor(testSpaceURL, testClientId, null, testMessageId, testAuthor, getTestAxiosOption())
            await expect(Achievement.getStarCountByUserId(testClientId, startOfDay, new Date(), testUserId)).resolves.toBe(0)
        })

        it('같은 글에 별을 줘도 점수가 추가되지 않는다', async () => {
            await sut.addPointToAuthor(testSpaceURL, testClientId, starGiver, testMessageId, testAuthor, getTestAxiosOption())
        ...
        it('하루에 가능한 별을 이미 다 보냈다면 점수가 추가되지 않는다', async () => {
            for (let i = 0; i < 5; i++) {
                await sut.addPointToAuthor(testSpaceURL, testClientId, starGiver, i.toString(), testAuthor, getTestAxiosOption())
            }
    ...
    describe('deleteStar 메소드에서', () => {
        it('점수가 없는 글을 리액션을 취소해도 에러가 발생하지 않는다', async () => {
            await expect(sut.deletePoint(testSpaceURL, testClientId, testMessageId, testUserId, getTestAxiosOption())).resolves.not.toThrow()
        })
        ...
        it('정상적으로 별을 취소하면 점수가 줄어든다', async () => {
            await sut.addPointToAuthor(testSpaceURL, testClientId, starGiver, testMessageId, testAuthor, getTestAxiosOption())
            await expect(Achievement.getStarCountByUserId(testClientId, startOfDay, new Date(), testUserId)).resolves.toBe(1)
            await expect(sut.deletePoint(testSpaceURL, testClientId, testMessageId, starGiver, getTestAxiosOption())).resolves.not.toThrow()
            await expect(Achievement.getStarCountByUserId(testClientId, startOfDay, new Date(), testUserId)).resolves.toBe(0)
        })
    })
})
